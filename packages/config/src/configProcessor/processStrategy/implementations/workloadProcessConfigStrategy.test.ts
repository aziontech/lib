...
describe('WorkloadProcessConfigStrategy', () => {
    ...
    describe('transformToConfig', () => {
        ...
        it('should transform multiple workload manifests to config format', () => {
            ...
            const result = strategy.transformToConfig(payload) as { workloads: Array<{ name: string }> };

            expect(result.workloads).toHaveLength(2);
            expect(result.workloads[0].name).toBe('workload-1');
            expect(result.workloads[1].name).toBe('workload-2');
        });
        ...
        it('should handle empty domains array', () => {
            ...
            const result = strategy.transformToConfig(payload) as { workloads: Array<{ domains: string[] }> };

            expect(result.workloads[0].domains).toEqual([]);
        });
        ...
        it('should handle undefined mtls', () => {
            ...
            const result = strategy.transformToConfig(payload) as { workloads: Array<{ mtls?: { verification: string; certificate?: number | null; crl?: number[] | null } }> };
