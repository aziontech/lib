...
function createConfigMismatchError(azionDir: string, svelteDir: string, suggestion?: string): Error {
  const message = [
    ...
    suggestion || 'Please update your svelte.config to match the Azion storage configuration.',
  ].join('\n');
...
async function prebuild(buildConfig: BuildConfiguration, ctx: BuildContext): Promise<AzionPrebuildResult | undefined> {
      ...
      if (!storage.dir && adapter.assetsDir) {
        if (normalizedSvelteDir !== PATHS.BUILD_DIR_DEFAULT) {
          throw createConfigMismatchError(
            ...
            normalizedSvelteDir,
            'Please change the assets option on svelte.config to match with Azion storage directory.',
          );
        }
      }
        ...
        if (normalizedStorageDir !== normalizedSvelteDir) {
          ...
          const suggestion =
            adapter.hasChangedAssetsDir || adapter.hasChangedPagesDir
              ? 'Please update your svelte.config assets or pages option to match the Azion storage configuration.'
              : 'Please update your svelte.config to match the Azion storage configuration.';
  ...
  if (adapter.hasAzion) {
    ...
    copyDirectory(PATHS.BUILD_DIR_DEFAULT, PATHS.STORAGE_DIR);
    try {
