...
export async function importPrivateKey(key: SignatureKey, alg: KeyImporterAlgorithm): Promise<CryptoKey> {
...
export async function exportPublicJwkFrom(privateKey: CryptoKey): Promise<JsonWebKey> {
...
export function getKeyAlgorithm(name: SignatureAlgorithm): KeyAlgorithm {
  switch (name) {
    case 'HS256':
      return {
        name: 'HMAC',
...
export function isCryptoKey(key: SignatureKey): key is CryptoKey {
